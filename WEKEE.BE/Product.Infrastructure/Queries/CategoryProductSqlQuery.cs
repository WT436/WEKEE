...
{
    public class CategoryProductSqlQuery
    {
        ...
        //public async Task<List<CategoryProductReadDto>> GetAllExactNotFTS(SearchOrderPageInput input)
        //{
        //    StringBuilder query = new StringBuilder();
        //    query.AppendLine("     SELECT                                                                                           ");
        //    query.AppendLine("  		CP.id                 AS 'Id',                                                              ");
        ...
        //        {
        //            if (i == 0)
        //                query.AppendLine($" WHERE {CategoryProductProperty.CONVERT_PROPERTY_CATEGORY(input.PropertySearch[0])} LIKE '%{input.ValuesSearch[0]}%' ");
        //            else
        //                query.AppendLine($" AND {CategoryProductProperty.CONVERT_PROPERTY_CATEGORY(input.PropertySearch[i])} LIKE '%{input.ValuesSearch[i]}%' ");
        //        }
        //    }
        //    query.AppendLine($"  ORDER BY {CategoryProductProperty.CONVERT_PROPERTY_CATEGORY(input.PropertyOrder)} {OrderByProperty.CONVERT_ORDER_BY(input.ValueOrderBy)} ");
        //    query.AppendLine($"  OFFSET(({input.PageIndex} - 1) * {input.PageSize}) ROWS                                                                        ");
