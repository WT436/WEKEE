...
namespace Account.Infrastructure.ADOQuery
{
    public class ResourceADO
    {
        ...
        public async Task<List<ResourceReadDto>> GetAllPageLstExactNotFTS(SearchOrderPageInput input)
        {
            ...
            query.AppendLine("FROM dbo.[Resource] AS R                                                                   ");
            ...
            if (input.PropertySearch != null)
            {
                ...
                {
                    if (i == 0)
                        query.AppendLine($" WHERE {ResourceTransform.CONVERT_SQL(key: input.PropertySearch[0], value: input.ValuesSearch[0])}");
                    else
                        query.AppendLine($" AND {ResourceTransform.CONVERT_SQL(key: input.PropertySearch[i], value: input.ValuesSearch[i])}");
                }
            }

            query.AppendLine($"  ORDER BY  {ResourceTransform.CONVERT_SQL(input.PropertyOrder)} {OrderByTransform.CONVERT_SQL(input.ValueOrderBy)}");
            query.AppendLine($"  OFFSET(({input.PageIndex} - 1) * {input.PageSize}) ROWS                                                                        ");
            query.AppendLine($"  FETCH NEXT {input.PageSize} ROWS ONLY                                                                            ");

            return await unitOfWork.FromSqlAsync<ResourceReadDto>(query.ToString());
        }
        ...
        public async Task<List<Resource>> GetResourcesById(List<int> ids)
        {
            StringBuilder query = new StringBuilder();
            query.AppendLine("SELECT      *            ");
            query.AppendLine("FROM dbo.[Resource] AS R ");
            ...
            return await unitOfWork.FromSqlAsync<Resource>(query.ToString());
        }
    }
}
