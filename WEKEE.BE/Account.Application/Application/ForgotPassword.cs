...
namespace Account.Application.Application
{
    public class ForgotPassword : IForgotPassword
    {
        ...
        public async Task<bool> TokenRequestAsync(ForgotPasswordDtos forgotPasswordDtos, string ip)
        {
            ...
            var listUserAccountIp = await userAccountIpQuery.GetListIpWithAccountAsync(accountDb.UserProfileId);
            var accountStatus = await userAccountStatusQuery.GetStatusWithId(accountDb.StatusId);
