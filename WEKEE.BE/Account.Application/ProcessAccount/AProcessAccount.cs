...
namespace Account.Application.ProcessAccount
{
    public class AProcessAccount : IProcessAccount
    {
        ...
        public async Task<UserLogin> ProcessTokenAsync(StringValues token, string ip)
        {
        ...
        public Task<JwtResponse> CreateTokenAsync(UserLogin userAccount, string ip)
        {
        ...
        public async Task<string> GetNameAccount(int id)
                 =>  (await userProfileQuery.GetUniqueId(idUser: id)).ZaloId;
    }
}