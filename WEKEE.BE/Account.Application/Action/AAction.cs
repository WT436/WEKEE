...
namespace Account.Application.Action
{
    public class AAction : IAction
    {
        ...
        public async Task<int> EditAction(ActionDto actionDto)
        {
            if (actionDto == null)
            {
            ...
            return await actionQuery.UpdateAsync(action);
        }
        ...
        public async Task<int> UpdateActionAsync(List<int> ids)
        {
            var dataAction = await actionQuery.GetDataByListId(ids: ids);
            return await actionQuery.UpdateAsync(dataAction.Select(m => { m.IsActive = !m.IsActive; return m; }).ToList());
        }
    }
}
