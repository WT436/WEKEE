...
import ConstRole, { confirmTypesRole } from "../objectValues/constRole";
import { RoleReadDto } from "../dto/roleReadDto";
import {
    deleteRoleStart, editStatusRoleStart, getRoleStart, insertOrUpdateRoleStart, searchSummaryRoleStart, summaryAtomicStart,
} from "../actions";
...
const stateSelector = createStructuredSelector<any, any>({
    loadingAll: makeLoadingRole(),
    loadingTable: makeLoadingTableRole(),
    loadingButton: makeLoadingButtonRole(),
    completedAll: makeCompletedRole(),
    pageIndex: makePageIndexRole(),
    pageSize: makePageSizeRole(),
    totalCount: makeTotalCountRole(),
    totalPages: makeTotalPagesRole(),
    roleData: makeRoleReads(),
    roleSummaryRead: makeroleSummaryRead()
});
export default function RoleComponent(props: IRoleComponentProps) {
    //#region START
    ...
    function _selectColumnSearch(value: number) {
        value === ConstRole.NULL || value === 0
            ? setpropertySearch([])
            : setpropertySearch([value]);
        setvaluesSearch([]);
        if (confirmTypesRole(value) === "DATE") {
        ...
        } else if (confirmTypesRole(value) === "SELECT") {
            setSelectColumnSearch(
                <Select
                    showSearch
                ...
                >
                    {(() => {
                        if (ConstRole[value] === ConstRole[ConstRole.CREATE_BY])
                        ...
                        // else if (
                        //     ConstRole[value] === ConstRole[ConstRole.TYPES_RSC]
                        // ) {
                        //     return Object.keys(ConstTypesRole).map((key: any) => {
                        //         if (!isNaN(Number(key))) {
                        //             return (
                        //                 <Option value={key}>
                        //                     {L(ConstTypesRole[key], "CONST_TYPE_ROLE")}
                        //                 </Option>
                        //             );
                        //         }
                        //     });
        ...
        } else if (confirmTypesRole(value) === "BOOLEAN") {
            setSelectColumnSearch(
                <Select
                    showSearch
                    placeholder={L("FILL", "COMMON")}
        ...
        } else if (confirmTypesRole(value) === "NUMBER") {
            setSelectColumnSearch(
                <InputNumber
        ...
        } else if (confirmTypesRole(value) === "STRING") {
            setSelectColumnSearch(
                <Input
                    placeholder={L("KEYWORD", "COMMON")}
                    onChange={(values: any) =>
    ...
    const _onChangeDataColumnSearch = (value: any) => {
        if (propertySearch[0] === ConstRole.NULL || value === 0) {
            setvaluesSearch([]);
        }
    ...
    const rowSelection = {
        onChange: (
            ...
            selectedRows: RoleReadDto[]
        ) => {
            setdataSelectFromTable(selectedRowKeys);
        },
        getCheckboxProps: (record: RoleReadDto) => ({
    ...
    const _removeItemSelect = () => {
        Modal.confirm({
            ...
            onOk: () => {
                dispatch(deleteRoleStart(dataSelectFromTable));
                _restartData();
            },
        });
    };
    ...
    const [dataBeginEdit, setdataBeginEdit] = useState<
        RoleReadDto | undefined
    >(undefined);
    const [form] = Form.useForm();

    const onFill = (value: RoleReadDto | undefined) => {
    ...
    const _onOkModalAddOrEdit = () => {
        form.validateFields().then((values) => {
            dispatch(insertOrUpdateRoleStart(values));
        });
    };
    //#endregion
    //#region CHART
    ...
    const _handleSearchModal = (value: any) => {
        ...
        timeout = setTimeout(() => dispatch(searchSummaryRoleStart({ text: value })), 1000);
    };
    ...
    return (
        <>
            ...
            <ChartComponent<yyy>
                ...
                title={L("TITLE_ALL_CHART", "CONST_TYPE_ROLE")}
                keyGen={["uv1", "uv2", "uv3", "uv4", "uv5", "uv0"]}
            />
            <Card
                title={L("TITLE_ALL_TABLE", "CONST_TYPE_ROLE")}
                size="small"
                type="inner"
            >
                <Row gutter={[10, 10]}>
                    <Col span={24}>
                        <Row gutter={[10, 10]}>
                            ...
                            <Col span={4}>
                                ...
                                >
                                    {Object.keys(ConstRole).map((key: any) => {
                                        if (!isNaN(Number(key))) {
                                            return (
                                                <Option value={key}>
                                                    {L(ConstRole[key], "CONST_TYPE_ROLE")}
                                                </Option>
                                            );
                                        }
                                    })}
                    ...
                    <Col style={{ margin: "10px 0", width: "100%" }}>
                        <Table
                            ...
                            rowKey={(record: RoleReadDto) => record.id}
                            loading={loadingTable}
                            style={{ width: "100%" }}
                            size="small"
                            pagination={{
            ...
            >
                <Form layout="horizontal" {...formItemLayout} form={form}>
                    ...
                    <Form.Item
                        label={L("ATOMIC_ID", "CONST_TYPE_ROLE")}
                        name="levelRole"
                        rules={[
                            {
                                required: true,
                    ...
                    >
                        ...
                        >
                            {roleSummaryRead.map((province: RoleSummaryReadDto) => (
                                <Option value={province.id}>{province.name}</Option>
                            ))}
                        </Select>
                    </Form.Item>
                    <Form.Item
                        label={L("DESCRIPTION", "CONST_TYPE_ROLE")}
                        name="description"
                        rules={[
                            {
                                required: true,
                    ...
                    <Form.Item
                        label={L("IS_ACTIVE", "CONST_TYPE_ROLE")}
                        name="isActive"
                    ...
                    <Form.Item
                        label={L("IS_DELETE", "CONST_TYPE_ROLE")}
                        name="isDelete"
                        valuePropName="checked"
                        rules={[
                            {
