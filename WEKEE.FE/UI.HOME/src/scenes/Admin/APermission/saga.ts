...
export default function* CallApiService() {
    ...
    yield takeLatest(
        ...
        function* (input: any) {
            try {
                const { output } = yield race({
                    output: call(async (): Promise<number> => {
                        let rs = await http.post('/PermissionAdminApi/AdminAddResourceFtPermission', input.payload);
                        return rs ? rs.data : rs;
                    })
                });
                if (output) {
    ...
    yield takeLatest(
        ActionTypes.ROLE_FT_PERMISSION_START,
        function* (input: any) {
            try {
                const { output } = yield race({
                    output: call(async (): Promise<PagedListOutput<RoleFtPermissionReadDto>> => {
                        let rs = await http.get('RoleAdminApi/AdminRoleFtPermission', { params: input.payload });
                        return rs ? rs.data : rs;
                    })
                });
                if (output) {
                    yield put(roleFtPermissionCompleted(output));
                }
                else {
                    yield put(roleFtPermissionError());
                }
            } catch (error) {
                yield put(roleFtPermissionError());
            }
    ...
    //#region SAVE_PERMISSION_FT_RESOURCE
    yield takeLatest(
        ActionTypes.SAVE_ROLE_FT_PERMISSION_START,
        function* (input: any) {
            try {
                const { output } = yield race({
                    output: call(async (): Promise<number> => {
                        let rs = await http.post('/RoleAdminApi/AdminRoleFtPermission', input.payload);
                        return rs ? rs.data : rs;
                    })
                });
                if (output) {
                    yield put(saveRoleFtPermissionCompleted(output));
                }
                else {
                    yield put(saveRoleFtPermissionError());
                }
            } catch (error) {
                yield put(saveRoleFtPermissionError());
            }
    ...
    //#endregion
     ...
     yield takeLatest(
        ActionTypes.SUBJECT_FT_ROLE_START,
        function* (input: any) {
            try {
                const { output } = yield race({
                    output: call(async (): Promise<PagedListOutput<SubjectFtRoleReadDto>> => {
                        let rs = await http.get('/SubjectAdminApi/AdminRoleFtSubject', { params: input.payload });
                        return rs ? rs.data : rs;
                    })
                });
                if (output) {
                    yield put(subjectFtRoleCompleted(output));
                }
                else {
                    yield put(subjectFtRoleError());
                }
            } catch (error) {
                yield put(subjectFtRoleError());
            }
    ...
    //#region SAVE_PERMISSION_FT_RESOURCE
    yield takeLatest(
        ActionTypes.SAVE_SUBJECT_FT_ROLE_START,
        function* (input: any) {
            try {
                const { output } = yield race({
                    output: call(async (): Promise<number> => {
                        let rs = await http.post('/SubjectAdminApi/AdminRoleFtSubject', input.payload);
                        return rs ? rs.data : rs;
                    })
                });
                if (output) {
                    yield put(saveSubjectFtRoleCompleted(output));
                }
                else {
                    yield put(saveSubjectFtRoleError());
                }
            } catch (error) {
                yield put(saveSubjectFtRoleError());
            }
