...
export default function FeatureProductComponent(
  props: IFeatureProductComponentProps
) {
  const dispatch = useDispatch();

  const { featureCartProduct } = useSelector(stateSelector);
  ...
  const _getImageAttribute = (item: string) => {
    return featureCartProduct.renderImage.filter(
      (element: ImageWithValueAttributeDto) => element.valueName === item
    )[0].imgS80x80;
  };

  const [MainFeatureCheckOne, setMainFeatureCheckOne] = useState<{
    key: string;
    value: string;
  }>();
  const [MainFeatureCheckTwo, setMainFeatureCheckTwo] = useState<{
    key: string;
    value: string;
  }>();
  ...
  useEffect(() => {
    let data: ProductReadForCartDto[] = [];
    featureCartProduct.productReadForCartDto.forEach(
      (el: ProductReadForCartDto) => {
        if (
          el.name === MainFeatureCheckOne?.key &&
          el.values === MainFeatureCheckOne?.value
        ) {
          data.push(el);
        }

        if (
          el.name === MainFeatureCheckTwo?.key &&
          el.values === MainFeatureCheckTwo?.value
        ) {
          data.push(el);
        }
      }
    ...
    dispatch(MainCheckFeatureStart(data.filter((e) => lookup.get(e.id) > 1)));
        ...
        <>
          <div className="wqPWTPYvMr">
            ...
            <div className="ofRgAVTeYl">
              {featureCartProduct.keyValuesName[0].valuesName.map(
                (item: string) => {
                  return (
                    <div
                      ...
                      onClick={() =>
                        _editCheckFeature(
                          featureCartProduct.keyValuesName[0].keyName,
                          item,
                          0
                        )
                      }
                    >
                      ...
                      {MainFeatureCheckOne !== undefined ? (
                        <span
                          style={{
                            display:
                              MainFeatureCheckOne.value == item
                                ? "block"
                                : "none",
                          }}
          ...
          <div className="wqPWTPYvMr">
            ...
            <div className="ofRgAVTeYl">
              {featureCartProduct.keyValuesName[1].valuesName.map(
                (item: string) => {
                  return (
                    <div
                      ...
                      onClick={() =>
                        _editCheckFeature(
                          featureCartProduct.keyValuesName[1].keyName,
                          item,
                          1
                        )
                      }
                    >
                      ...
                      {MainFeatureCheckTwo !== undefined ? (
                        <span
                          style={{
                            display:
                              MainFeatureCheckTwo.value == item
                                ? "block"
                                : "none",
                          }}
                          className="nuTLGkaQio"
