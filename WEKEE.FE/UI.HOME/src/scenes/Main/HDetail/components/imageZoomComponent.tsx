...
export default function ImageZoomComponent(props: IImageZoomComponentProps) {
  ...
  const { imageForProduct, mainFeatureCheck } = useSelector(stateSelector);
  //#endregion
    ...
    imageForProduct.forEach((element: ImageForProductDto) => {
      if (element.size == "S80x80") {
        ...
        if (element.displayOrder === 1) {
          dataZoomImage.largeImage.src =
            abp.serviceAlbumImage + element.virtualPath;
        }
      } else {
        setImageS340x340((ImageS340x340) => [...ImageS340x340, element]);
        if (element.displayOrder === 1) {
          setselectImage(element.imageRoot);
          dataZoomImage.smallImage.src =
            abp.serviceAlbumImage + element.virtualPath;
  ...
  const _selectImageShow = (imageRoot: number) => {
    ...
    var image1360Show = ImageS1360x540.find(
      (item: ImageForProductDto) => item.imageRoot === imageRoot
    );
    var image340Show = ImageS340x340.find(
      (item: ImageForProductDto) => item.imageRoot === imageRoot
    );
    setzoomImage({
      smallImage: {
        ...
        isFluidWidth: true,
        src:
          image340Show === undefined
            ? ""
            : abp.serviceAlbumImage + image340Show.virtualPath,
      },
      largeImage: {
        src:
          image1360Show === undefined
            ? ""
            : abp.serviceAlbumImage + image1360Show.virtualPath,
        width: 900,
        height: 900,
      },
    });
  ...
  //#region FEATURE CHANGE
  useEffect(() => {
    // select root image
    let imageS80x80 = mainFeatureCheck[0]?.imgS80x80;
    // call _selectImageShow
    imageForProduct.forEach((el: ImageForProductDto) => {
      if (el.size === "S80x80" && el.virtualPath == imageS80x80) {
        _selectImageShow(el.imageRoot);
      }
    });
  }, [mainFeatureCheck]);
