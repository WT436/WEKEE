...
export default function HDetail(props: IHDetailProps) {
  ...
  useEffect(() => {
    if (Key1State && Key2State) {
      var data = unitCardProduct.featureProductCardDtos.filter(
        (x: FeatureProductCardDtos) =>
          x.properties1.replace(" ", "") === Key1State.name.replace(" ", "") &&
          x.properties2.replace(" ", "") === Key2State.replace(" ", "")
      )[0];
      setprice(data?.price);
      ...
      if (Key1State) {
        var data = unitCardProduct.featureProductCardDtos.filter(
          (x: FeatureProductCardDtos) =>
            x.image === Key1State.imageRoot && x.properties1 === Key1State.name
        )[0];
        setprice(data?.price);
        setpriceMarket(data?.priceMarket);
        onSelectImageZoom(data.image);
      ...
      if (Key2State) {
        var data = unitCardProduct.featureProductCardDtos.filter(
          (x: FeatureProductCardDtos) => x.properties2 === Key2State
        )[0];
        setprice(data?.price);
        setpriceMarket(data?.priceMarket);
        onSelectImageZoom(data.image);
      ...
      <Row>
        <Col span={7} className="lIiNahOSjz">
                ...
                return (
                  <img
                    className={
                      "LmqLbjIyYc" +
                      " " +
                      (selectImage === element.id ? "bFpTBTrDfH" : "")
                    }
                    onClick={() => onSelectImageZoom(element.imageRoot)}
          ...
          <Row className="qFCWRAtWva">
            ...
            <Col span={18}>
              <a className="XhuOxcXgNZ" href="#">
                {" "}
                Bé mèo vàng
              </a>
              <div style={{ display: "flex", width: "100%" }}>
                <Button
                  className="PhtnSETPFW"
                  size="small"
                  icon={<ShopOutlined />}
                >
                  Shop
        ...
        <Col style={{ width: "calc(100% - 340px)", paddingLeft: "10px" }}>
          <div className="LboUrliYOw">
            <div className="RnmeRAZZRs">
              Thương hiệu :{" "}
              <a>{unitCardProduct.productCardDtos.nameTrademark}</a> | Đứng thứ
              3 trong <a href="">Top 1000 Áo thun nữ bán chạy tháng này</a>
            </div>
            <div className="EQjksUFMJC">
              {unitCardProduct.productCardDtos.name}
            </div>
          ...
          <Row>
                    ...
                    return (
                      ...
                      >
                        <img
                          src={
                            abp.serviceAlbumImage +
                            unitCardProduct.imageProductCardDtos.filter(
                              (x: ImageProductCardDtos) =>
                                x.imageRoot === item.img && x.size === "S80x80"
                            )[0].src
                          }
                          alt=""
                        />
                        ...
                        <span
                          style={{
                            display:
                              Key1State?.name === item.value ? "block" : "none",
                          }}
                  ...
                  {startValues2.map((item: string) => {
                    return (
                      <div
                        ...
                        onClick={() => setKey2State(item)}
                      >
                        <span>{item.replace(" ", "")}</span>
                        <span
                          style={{
                            display: Key2State === item ? "block" : "none",
                          }}
                          className="nuTLGkaQio"
                        >
                          <CheckOutlined className="SWzJAVZYWg" />
              ...
              <div>
                ...
                <div className="hcUCOccMik">
                  <span>
                    ...
                    nội <p onClick={() => setVisible(true)}>đổi địa chỉ</p>
                  </span>
                </div>
                <Modal
            ...
            <Col className="otSUEGfNRF">
              <ul className="UPkAKNmJzp">
                {unitCardProduct.highlightProductCardDtos.map(
                  (elment: HighlightProductCardDtos) => {
                    return (
